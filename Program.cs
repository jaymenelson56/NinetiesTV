...
static List<Show> AllButWorst(List<Show> shows)
{
    ...
    var lowestRatedShow = shows.OrderBy(show => show.ImdbRating).FirstOrDefault();

    
    return shows.Where(show => show != lowestRatedShow).ToList();
}
}
...
{
    return shows.Where(s  => s.Genres.Count > 1)
...
{
    var orderedShows = shows.OrderBy(s => s.EndYear);
...
{
    var orderedShows = shows.OrderByDescending(s => s.ImdbRating);
...
{
    var highestRatedShows = shows.Where(s => s.Genres.Contains("Drama"))
    .OrderByDescending(s => s.ImdbRating)
    .FirstOrDefault();

    var dramasExceptBest = shows.Where(s => s.Genres.Contains("Drama") && s != highestRatedShows).ToList();

    return dramasExceptBest;
}
...
static Show WordieastName(List<Show> shows)
{
    ...
    int maxWordCount = shows.Max(s => s.Name.Split(' ', StringSplitOptions.RemoveEmptyEntries).Length);

    
    var showsWithMaxWords = shows.Where(s => s.Name.Split(' ', StringSplitOptions.RemoveEmptyEntries).Length == maxWordCount);

    return showsWithMaxWords.FirstOrDefault();;
}
...
{
    var showNames = shows.Select(s => s.Name);
...
static string AllNamesWithCommasPlsAnd(List<Show> shows)
{
    ...
    if (showNames.Count() < 2)
        return string.Join("", showNames);

        return String.Join(", ", showNames.Take(showNames.Count() - 1 )) + " and " + showNames.Last();
}
